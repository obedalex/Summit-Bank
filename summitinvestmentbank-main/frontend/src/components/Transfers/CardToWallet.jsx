...
import { CreditCard, Wallet, Send, KeyRound } from "lucide-react";
...
export default function CardToWallet() {
  ...
  ============================ */
  const handleSend = () => {
    if (!fromCard) return toast.error("Select card");
  ...
  const validatePin = () => {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        toast.success("Card → Wallet transfer successful!");
  ...
  return (
    <div className="w-full max-w-lg mx-auto mt-5">
      <div className="bg-white/80 shadow-xl backdrop-blur-xl border rounded-2xl p-6 space-y-6">

        <h2 className="text-xl font-semibold flex items-center gap-2">
          <Wallet size={20} /> Card → Wallet Transfer
        </h2>
        ...
        <button
          ...
          className="w-full py-3 rounded-xl text-white bg-gradient-to-r from-[#4456ff] to-[#1e30fe]
                     font-semibold flex items-center justify-center gap-2 shadow-md
                     hover:scale-[0.99] transition-all disabled:opacity-40"
        >
          {sendLoading ? <ClipLoader size={18} color="white" /> : <>
  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        className="mt-1 p-3 bg-gray-50 border rounded-xl flex items-center justify-between cursor-pointer"
      >
        {selected ? (
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 5 }}
            className="absolute w-full mt-2 bg-white border rounded-xl shadow-xl max-h-64 overflow-y-auto z-50"
          >
            {cards.map((card) => (
              <div
                key={card._id}
      ...
      >
        <div className="w-full max-w-sm bg-white p-6 rounded-2xl shadow-xl">
          ...
          <div className="mt-5 flex gap-3">
            ...
            <button
              onClick={onConfirm}
              className="flex-1 py-2 rounded-xl bg-gradient-to-r from-[#4456ff] to-[#1e30fe] 
